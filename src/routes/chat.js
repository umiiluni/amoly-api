    ...
    const accessToken = authHeader.replace(/^Bearer\s+/i, '');
              ...
              if (!accessToken) {
                    return res.status(401).json({
                            error: 'Falta el token de autenticacion. Envia el access_token de Supabase del usuario logueado en el header Authorization: Bearer <token>.',
                    });
              }

              const bodyMessage = (req.body || {}).message;
    const message = bodyMessage || req.query.message;
    if (!message || typeof message !== 'string') {
          return res.status(400).json({ error: 'Falta el mensaje: envia "message" en el body JSON o como query param ?message=.' });
    }
